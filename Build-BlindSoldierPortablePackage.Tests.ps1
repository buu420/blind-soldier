...
function New-PortableFixture {
    ...
    [IO.File]::WriteAllText((Join-Path $mod 'ModConfig.json'),
        '{"ModId":"ff7.accessibility.reloaded","ModVersion":"0.1.8","ModR2RManagedDll32":"x86/Ff7.Accessibility.Reloaded.dll","ModR2RManagedDll64":"x64/Ff7.Accessibility.Steam2026X64.dll","ModDependencies":["reloaded.sharedlib.hooks"],"SupportedAppId":["ff7_en.exe","ff7.exe","FFVII.exe"]}')
...
function Invoke-FixtureBuild {
    ...
    & $builderPath -OutputPath $Output -Version '0.1.8' `
...
function New-PortableArchiveVariant {
    ...
    try {
        ...
        [ordered]@{schemaVersion=1;version='0.1.8';files=$records} |
...
Describe 'Blind Soldier direct-extract portable package' {
    ...
    It 'rejects an extra nested Windows-canonical Version proxy path' {
        ...
        { & $verifierPath -ArchivePath $variant -ExpectedVersion '0.1.8' } |
    ...
    It 'allows Blind Soldier owned paths whose leaf matches an FFNx directory prefix' {
        ...
        $result = & $verifierPath -ArchivePath $variant -ExpectedVersion '0.1.8'
        $result.Version | Should Be '0.1.8'
    }
    ...
    It 'passes the production verifier with the real Version proxy fixture' {
        $fixture = New-PortableFixture
        Invoke-FixtureBuild -Fixture $fixture -Output $fixture.First
        $result = & $verifierPath -ArchivePath $fixture.First -ExpectedVersion '0.1.8'
        $result.Version | Should Be '0.1.8'
    ...
    It 'rejects an unsafe ZIP member before extraction' {
        ...
        { & $verifierPath -ArchivePath $fixture.First -ExpectedVersion '0.1.8' } |
