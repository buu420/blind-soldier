...
# mds7pb_1's border1 and border4 and mkt_s1's line01 start each visit switched off (LINOFF in
# their Main, or in line01's Init unless 1[160] bit 7 and 1[161] & 0xE0 are set before moment
# 192), and only their own Move script runs these steps, so each is offered while its line is on.
Add-Definition -FieldId 154 -FieldName 'mds7pb_1' -Kind 'Location' -Label 'Approach the front door; Barret and Avalanche are entering' -X 264 -Y 0 -Z 0 -TargetGameMoment 72 -MinimumGameMoment 69 -MaximumGameMoment 71 -Priority 0 -CompletedCondition (New-Condition 3 212 0x01 0x01) -EntityName 'border1' -ScriptType 'Move' -RequiredEnabledLineEntityId 14 -TriggerLine ([ordered]@{ startX = 255; startY = -48; startZ = 0; endX = 274; endY = 48; endZ = 0 })
...
Add-Definition -FieldId 154 -FieldName 'mds7pb_1' -Kind 'Location' -Label 'Walk toward the front door; Tifa will stop Cloud about the promise' -X 50 -Y -24 -Z 0 -TargetGameMoment 84 -MinimumGameMoment 78 -MaximumGameMoment 83 -Priority 0 -RequiredCondition (New-Condition 3 213 0x01 0x01) -CompletedCondition (New-Condition 3 213 0x08 0x08) -EntityName 'border4' -ScriptType 'Move' -RequiredEnabledLineEntityId 17 -TriggerLine ([ordered]@{ startX = 51; startY = 48; startZ = 0; endX = 49; endY = -96; endZ = 0 })
...
# colne_6's bedroom. 3[162] says who Corneo chose: 0x10 Cloud, 0x20 Tifa, 0x40 Aeris. AD's Main
# switches the rug's LINE (entity 16, the trapdoor) off on entry (REQ 16 script 7, LINOFF), and
# AD2's script 5 is what switches it on again (REQEW 16 script 8) once the scene has played, so
# the rug is a step only while that line is on. For Tifa or Aeris the scene plays by itself on
# entry. For Cloud it does not: Corneo's own Talk is empty, and what starts it is BLINE (entity
# 18) beside the bed, whose [OK] runs his conversation (REQEW 12 script 12) and then AD2's
# scripts 7 and 5. BLINE's Main keeps it on only while 3[162] bit 0x10 is set and 3[163] bit
# 0x02 is clear, and its [OK] switches it off.
# BLINE's [OK] (00637D35) runs only while the leader touches the line, inside its own collision
# radius (00637ABB), so the step is reached on that touch, not at the player's arrival distance.
Add-Definition -FieldId 211 -FieldName 'colne_6' -Kind 'Location' -Label "Go to Corneo's bedside and press OK to talk to him" -EntityId 18 -X 128 -Y 288 -Z -237 -MinimumGameMoment 197 -MaximumGameMoment 197 -Priority 0 -RequiredCondition (New-Condition 3 162 0x10 0x10) -RequiredEnabledLineEntityId 18 -EntityName 'BLINE' -ScriptType '[OK]' -UsesPlayerCollisionRadius -TriggerLine ([ordered]@{ startX = 128; startY = 368; startZ = -237; endX = 128; endY = 207; endZ = -237 }) -KeepActiveOnArrival
Add-Definition -FieldId 211 -FieldName 'colne_6' -Kind 'Location' -Label 'Cross the rug in the middle of the bedroom' -X 38 -Y 21 -Z -237 -TargetGameMoment 203 -MinimumGameMoment 197 -MaximumGameMoment 197 -Priority 0 -RequiredEnabledLineEntityId 16 -EntityName 'LINE' -ScriptType 'Move' -TriggerLine ([ordered]@{ startX = 0; startY = 83; startZ = -237; endX = 76; endY = -41; endZ = -237 })
...
. (Join-Path $scriptRoot 'story-regions\WutaiPagoda.ps1')
