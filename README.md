...
Version 0.7.2 gives the Temple of the Ancients clock brief clock-time speech, such as
"Moving, ten thirty." and "Stopped, ten thirty.", instead of long queued descriptions.
The newest reading replaces the last one, the Time Guardian's words are not cut off,
and R still gives the full detail of hands, place and bridges. The clock has not yet
been checked in a live playthrough. See the [release notes](docs/releases/v0.7.2.md).

Version 0.7.1 fixes the Added Effect return route, Corneo's bedside conversation,
and stopping within the game's actual interaction range at counters and background
objects. It adds missing materia-cave crystals and corrects several scripted
approaches. The guide audit covers all 43 walkthrough chapters against native field
data in both runtimes; it is not a complete live playthrough. See the
[release notes](docs/releases/v0.7.1.md) and
[audit ledger](docs/guide-route-audit-ledger-20260925.md) for the scope and remaining
unverified states.
...
additional Story and interaction steps in Cosmo Canyon, Gold Saucer, Junon,
Fort Condor, Lucrecia's cave and the later Corel pursuit. All 702 mapped fields
were audited and all 280 Story checkpoints passed; live playthroughs and some
optional-object approaches still need verification. See the
[release notes](docs/releases/v0.6.9.md) for the scope and remaining limits.
...
records boarding contact, updates its approach if the vehicle moves, and stops
steering if the target disappears or is boarded. Both runtime regression suites
and 12 simulated movement cases passed; live boarding still needs testing.
...
dial so it cannot interrupt the numbers. Use normal game speed for this timed
puzzle. Automated checks passed; live safe gameplay still needs confirmation.
...
- [Blind-Soldier-Portable.zip](https://github.com/buu420/blind-soldier/releases/download/v0.7.2/Blind-Soldier-Portable.zip)
...
- [Blind-Soldier-2013-x86-Portable.zip](https://github.com/buu420/blind-soldier/releases/download/v0.7.2/Blind-Soldier-2013-x86-Portable.zip)
